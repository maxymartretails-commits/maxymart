...
const DeleteProductModal: React.FC<DeleteProductModalProps> = ({
  ...
  const handleDelete = async () => {
    await deleteProducts({id:details?.id})
      .then(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 border border-gray-200">
        ...
        <div className="mt-6 flex justify-center gap-4">
          ...
          <button onClick={handleDelete} className="px-5 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium shadow-md">
            Delete
          </button>
        </div>
      </div>
